...
export default function ImportStudentsModal({
  ...
  const validateRow = (row: string[]): ImportPreview => {
    const [matricule, lastName, firstName, className] = row.map(s => s.trim());
    const cycle = className?.startsWith('L') ? 'LG' : 'LEP';
    
    let isValid = true;
    let error = '';

    if (!matricule || !lastName || !firstName || !className) {
      isValid = false;
      error = 'Tous les champs sont obligatoires';
    } else if (store.getStudent(matricule)) {
      isValid = false;
      error = 'Un élève avec ce matricule existe déjà';
    }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!header.includes('matricule') || !header.includes('nom') || 
            !header.includes('prenom') || !header.includes('classe')) {
          setError("L'en-tête du fichier doit contenir: matricule, nom, prenom, classe");
          return;
        }
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="bg-gray-50 p-3 rounded-md">
                      <code className="text-sm">matricule,nom,prenom,classe</code>
                    </div>
                  </div>
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {preview.map((row, index) => (
                              <tr key={index} className={row.isValid ? '' : 'bg-red-50'}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">{row.matricule}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">{row.lastName}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">{row.firstName}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">{row.class}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm">{row.cycle}</td>
